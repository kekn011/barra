#!/usr/bin/env python3
"""cmp_stages.py — Stufen-Bisektion der v2.1-Geraete-Dumps gegen numpy-Nachrechnung aus dump_h_f32."""
...
x8=rd("x_i8.bin",np.int8).reshape(S,D).astype(np.float32)
q8=rd("q_i8.bin",np.int8).reshape(S,H*HD).astype(np.float32)
k8=rd("k_i8.bin",np.int8).reshape(S,NKV*HD).astype(np.float32)
v8=rd("v_i8.bin",np.int8).reshape(S,NKV*HD).astype(np.float32)
qk=rd("qk_f16.bin",np.float16).astype(np.float32)
aof=rd("aof_f16.bin",np.float16).astype(np.float32).reshape(S,H*HD)
ao8=rd("ao_i8.bin",np.int8).reshape(S,H*HD).astype(np.float32)
o8=rd("o_i8.bin",np.int8).reshape(S,D).astype(np.float32)
...
# Stufe 0: srow korrekt?
mx=np.abs(h).max(-1); mx[mx==0]=1
cosd("srow (CPU-Zeilenmax)",srow,mx)
# Stufe 1: x_i8 Dequant vs h
hq=(x8-m[1])*m[0]*srow[:,None]
cosd("h nach int8 (dequant)",hq,h)
# Stufe 2: TPU q/k/v vs numpy (auf Basis hq)
qexp=hq@Wq.T; kexp=hq@Wk.T; vexp=hq@Wv.T
qd=(q8-m[3])*m[2]*srow[:,None]; kd=(k8-m[7])*m[6]*srow[:,None]; vd=(v8-m[11])*m[10]*srow[:,None]
cosd("TPU q (dequant vs numpy)",qd,qexp)
cosd("TPU k",kd,kexp)
cosd("TPU v",vd,vexp)
...
# Stufe 5: ctx-Quant
cm=np.abs(aof).max(-1); cm[cm==0]=1
cosd("srow_c",srowc,cm)
aoq=(ao8-m[13])*m[12]*srowc[:,None]
cosd("ctx nach int8 (dequant)",aoq,aof)
# Stufe 6: TPU o
oexp=aoq@Wo.T
od=(o8-m[15])*m[14]*srowc[:,None]
