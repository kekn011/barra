#!/usr/bin/env python3
"""gen_attn_pkgs4.py — ggml-barra v2.3: 16x8-Packages (Aktivierungen int16 symmetrisch, Gewichte int8).
Bei ±32767 Stufen braucht es KEINE Zeilennorm — schlichte per-tensor-Kalibrierung auf echten Daten.

  python3 gen_attn_pkgs4.py <gguf> <outdir> [S=512]

Schreibt nach <outdir>:
  q_L{i}.tflite k_L{i}.tflite v_L{i}.tflite o_L{i}.tflite   (i=0..NL-1)
  attn4.meta : Zeile 1 "D H NKV HD FF NL S 3"  (Version 3 = int16-I/O), je Layer 16 Werte wie v2
  aux_attn.bin : wie v2.x (gamma_q[NL][HD], gamma_k[NL][HD], cos, sin)
"""
...
for i in range(NL):
    ...
    o=ctx2@Wo.T
    qq=conv_pkg(f"q_L{i}",Wq,D,H*HD,h)
    kq=conv_pkg(f"k_L{i}",Wk,D,NKV*HD,h)
    vq=conv_pkg(f"v_L{i}",Wv,D,NKV*HD,h)
    oq=conv_pkg(f"o_L{i}",Wo,H*HD,D,ctx2)
    m=[]
...
with open(os.path.join(OUT,"attn4.meta"),"w") as f:
    f.write(f"{D} {H} {NKV} {HD} {FF} {NL} {S} 3\n")
    for m in meta:
