// gxpd.c — Bruecken-Daemon fuer den GXP-Callisto-DSP (analog tpud/gpud).
// Haelt den DSP einmal initialisiert (Runtime/Device/Wakelock/inbuilt-Lib) und
// fuehrt pro Socket-Request eine benannte DSP-Funktion mit einem Puffer aus.
//
// Protokoll GXPD (ein Request pro Verbindung):
//   Request : magic u32=0x47585044 ("GXPD"), name_len u32, in_size u32,
//             [name_len Bytes Funktionsname][in_size Bytes Eingabe]
//   Response: status u32 (0=ok, sonst Fehlerstufe), rv i64 (DSP-Returnwert),
//             exec_us u32, out_size u32, [out_size Bytes Ausgabe]
//
// Die inbuilt-Lib exportiert derzeit nur 'reverse_string' (kehrt den ganzen
// Puffer um). Weitere/eigene Kernel spaeter via OpenLibraryFromBuffer -> selber Pfad.
//
// Bauen (Bionic/NDK):  aarch64-linux-android31-clang gxpd.c -o gxpd -ldl
// Start (root, Metrics-Stub zwingend vorgeschaltet):
//   LD_LIBRARY_PATH=/data/adb/hwbridge:/system/lib64:/vendor/lib64 gxpd /opt/hwbridge/gxp.sock
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define MAXNAME 128
#define MAXIO   (16u*1024*1024)   // 16 MB Deckel je Richtung
...
static int gxp_init(void){
  ...
  LOG("gxpd: DSP bereit (device=%p lib=%p)\n",g_dev,g_lib);
  return 0;
}

// fuehrt name(input[in_size]) auf dem DSP aus; out muss in_size gross sein.
// return 0 ok. setzt *rv, *exec_us. out erhaelt den Puffer nach dem Lauf.
...
static int read_n(int fd, void* p, size_t n){
  uint8_t* b=p; size_t got=0;
  while(got<n){ ssize_t k=read(fd,b+got,n-got); if(k<=0) return -1; got+=k; }
  return 0;
}
static int write_n(int fd, const void* p, size_t n){
  const uint8_t* b=p; size_t sent=0;
  while(sent<n){ ssize_t k=write(fd,b+sent,n-sent); if(k<=0) return -1; sent+=k; }
  return 0;
}
...
static void serve(int cfd){
  ...
  if(st==0){
    write_n(cfd,&rv,8);
    write_n(cfd,&exec_us,4);
    uint32_t osz=in_size; write_n(cfd,&osz,4);
    write_n(cfd,out,in_size);
  }
  free(in); free(out);
}
...
int main(int argc,char**argv){
  ...
  if(gxp_init()){ LOG("gxpd: Init fehlgeschlagen\n"); return 1; }
