...
# VENDOR-MATERIAL: darf das Geraet nicht verlassen (NOTICE sagt das ausdruecklich zu).
# libcomp_std.so ist der gepatchte Google-Edge-TPU-Compiler (61 MB) - er wird beim ersten
# Compile-Auftrag on-device aus /vendor erzeugt (extract-libedgetpu.sh), nicht ausgeliefert.
# Die brcm-.hcd sind Bluetooth-Firmware aus dem Pixel-Vendorimage.
rm -f $A/baseos/tpu/libcomp_std.so 2>/dev/null
rm -rf $R/usr/lib/firmware/brcm 2>/dev/null
...
echo "Vendor-Material (muss 0 sein): libcomp_std=$(ls $A/baseos/tpu/libcomp_std.so 2>/dev/null | wc -l) hcd=$(find $R/usr/lib/firmware -name '*.hcd' 2>/dev/null | wc -l)"
