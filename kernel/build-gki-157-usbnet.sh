...
echo "== Fester Modul-Signaturschluessel (30.8.) =="
# Jeder Build erzeugt sonst einen neuen Zufallsschluessel; GKI lehnt fremd signierte Module mit
# geschuetzten Exporten ab (rfkill.ko -> ohne WLAN). Mit festem Schluessel bleiben rfkill.ko und die
# ipset/vxlan-Module ueber Kernel-Updates hinweg gueltig. Schluessel liegt NICHT im Repo:
# $T/keys/barra_module_signing_key.pem (Kopie nach aosp/certs/ fuer den Bazel-Sandkasten).
KEY="$T/keys/barra_module_signing_key.pem"
# Kleaf (define_common_kernels) setzt CONFIG_MODULE_SIG_KEY beim Build zurueck - die Zeile hier hat am
# 30.8. NICHT gegriffen (Module tragen "Build time autogenerated kernel key"). Offen: kernel_build
# direkt mit module_signing_key aufrufen. Bis dahin: Kernel und Module IMMER als Satz ausliefern.
[ -f "$KEY" ] || echo "HINWEIS: $KEY fehlt (Schluessel-Weg noch nicht wirksam, s. Kommentar)"
